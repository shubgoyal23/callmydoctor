...
const getDoctorsSlots = asyncHandler(async (req, res) => {
    ...
    const slots = await redisClient.lRange(`doctor:${id}:${date_formated}`, 0, -1);
    const timeAvailable = doctor.availability.find((item) => item.day === daysOfWeek[d.getDay()]);
    if (!timeAvailable) {
...
const bookAppointment = asyncHandler(async (req, res) => {
    ...
    sendEmail(req.user?.email, "Appointment booked successfully", "Appointment booked successfully");
    return res
        .status(200)
        .json(new ApiResponse(200, appointment, "Appointment booked successfully, please wait for confirmation"));
});
...
export { getDoctorList, getDoctorsSlots, bookAppointment, getAppointmentList, getAppointmentDetails, };
